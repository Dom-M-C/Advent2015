...
getDim Length (Dim x _ _) = x
getDim Width (Dim _ x _) = x
getDim  Height(Dim _ _ x) = x
...
parseInput :: [[Char]] -> [PresentDim]
parseInput [] = []
parseInput (s:sx) =   (parseListToDim (map (\x ->  parseChar x) (splitOn "x" s))) : (parseInput sx)

parseChar :: [Char] -> Int
parseChar (c:cs)
    | isNumber c = read (c:cs)::Int
    | otherwise = 0

parseListToDim :: [Int] -> PresentDim
parseListToDim cs
    | length cs /= 3 = Dim 0 0 0
    | otherwise =
        let
            w = cs !! 0
            h = cs !! 1
...
firstAnswer = parseTotal withInput
secondAnswer = parseRibbons withInput

withInput = splitOn ","  "20x3x11 \
\,15x27x5                         \
\,6x29x7                          \
\,30x15x9                         \
\,19x29x21                        \
